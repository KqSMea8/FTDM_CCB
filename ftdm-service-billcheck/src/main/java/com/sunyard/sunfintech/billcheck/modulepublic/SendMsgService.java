        ...
        try{
            SingleThreadPool.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    accOpenconfigExample.createCriteria().andPlat_noEqualTo(plat_no).andAccount_typeEqualTo(AccountType.Platcust.getCode()).
                            andSub_subjectEqualTo(Ssubject.INVEST.getCode()).andSubjectEqualTo(Tsubject.CASH.getCode());
                    List<AccOpenconfig> accOpenconfigList = accOpenconfigMapper.selectByExample(accOpenconfigExample);
                    if(accOpenconfigList.size()>0){
                        String mall_no=accOpenconfigList.get(0).getMall_no();
...
//                });
    ...
    public BigDecimal getAccountAllAmount(String platcust) throws BusinessException {
        ...
        criteria.andEnabledEqualTo(Constants.ENABLED);
        List<AccountSubjectInfo> currentAccountList= new ArrayList<AccountSubjectInfo>();//TODO 杨磊 accountSubjectInfoMapper.selectByExample(accountSubjectInfoExample);
    ...
    public void sendMail(String content, String mail_subject) throws BusinessException {
        try{
            ...
            if(CacheUtil.getCache().setnx(mail_subject+content, DateUtils.todayStr())){
                ...
                logger.error("outside模块被删除，无法发送邮件");
                //iOutsideService.sendMail(mail_subject,content);
            }
        }catch (Exception e){
