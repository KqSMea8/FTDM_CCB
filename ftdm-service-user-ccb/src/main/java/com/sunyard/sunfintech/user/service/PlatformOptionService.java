    ...
    public BaseResponse transferToPerson(TransferToPersonRequest transferToPersonRequest) throws BusinessException {
        ...
        try {
            ...
            try{
                ...
                String msgContent=String.format("平台转个人资金回滚失败，请人工处理|订单号：%s|流水号：%s",transTransReq.getOrder_no(),transTransReq.getTrans_serial());
                sendMsgService.sendErrorToAdmin(msgContent,transTransReq.getPlat_no());
                logger.error("【平台转个人】转账回滚异常");
            }
            baseResponse.setRecode(BusinessMsg.DATEBASE_EXCEPTION);
...
//                newAccountTransferService.singleCharge(eaccAccountamtlist);
            ...
            //支付异步通知可能早，所以防止重复更新rwRecharge表数据，先查再改
...
/*            rwRecharges.get(0).setReturn_code(OrderStatus.SUCCESS.getCode());
    ...
    public WithdrawResponse withdraw(WithdrawRequest withdrawRequest)throws BusinessException{
        ...
        try{
            ...
            if(BigDecimal.ZERO.compareTo(accountSubjectInfo.getN_balance()) >= 0 ){
            ...
            String bd100=String.valueOf(withdrawRequest.getAmount().multiply(new BigDecimal(100)));
            ...
            if(StringUtils.isNotBlank(singleLimited)){
                ...
                if(StringUtils.isNotBlank(allLimited)){
                    ...
                    //查询出当日全部的平台提现
                    long platWithdrawLimitedX100=CacheUtil.getCache().increment(platWithdrawRedisKey,Long.parseLong(bd100.indexOf(".")>0?bd100.substring(0,bd100.indexOf(".")):bd100));
                    CacheUtil.getCache().expire(platWithdrawRedisKey,24*3600);
                    if(allLimitedBD.compareTo(new BigDecimal(platWithdrawLimitedX100).divide(new BigDecimal(100)))<0){
                        CacheUtil.getCache().increment(platWithdrawRedisKey,0L-Long.parseLong(bd100.indexOf(".")>0?bd100.substring(0,bd100.indexOf(".")):bd100));
                        //当日平台提现总额超限
                        throw new BusinessException(BusinessMsg.LIMIT_OUT,BusinessMsg.getMsg(BusinessMsg.LIMIT_OUT));
                    }
            ...
            String finStatus=OrderStatus.REQUESTSUCCESS.getCode();
            if("BOB".equals(deployEnvironment)){
                //BOB版本，因为行内支付直接返回终态，故无需冻结，直接转账
                ...
                try{
                    ...
                    eaccTransTransreqWithBLOBs.setTrans_serial(SeqUtil.getSeqNum());
                    ...
                    finStatus=OrderStatus.SUCCESS.getCode();
                }catch (Exception e){
                    finStatus=OrderStatus.FAIL.getCode();
                    //退款
                    eaccAccountamtlist.setAmt_type(AmtType.INCOME.getCode());
                    eaccAccountamtlist.setRemark("平台提现退款");
                    accountAssetService.charge(eaccAccountamtlist);
                }
            }else{
                //行内划转pay_code 020
                //CCB版本，因为支付不返回终态，故需要冻结资金
                accountFrozenService.freeze(eaccAccountamtlist);
                ...
                try{
                    ...
                    String ownUrl = sysParameterService.querySysParameter(SysParamterKey.FTDM, URLConfigUtil.PLAT_WITHDRAW_NOTIFY_URL);
                    ownUrl = host + ownUrl;
                    ...
                    if(OrderStatus.FAIL.getCode().equals(resultMap.get("order_status"))){
                        ...
                        eaccAccountamtlist = accountTransferService.queryTransFlow(eaccAccountamtlist);
                        accountFrozenService.unfreeze(eaccAccountamtlist);
                    }else if(OrderStatus.SUCCESS.getCode().equals(resultMap.get("order_status"))){
                        finStatus=OrderStatus.SUCCESS.getCode();
                    }else{
                        finStatus=OrderStatus.REQUESTSUCCESS.getCode();
                    }
                }catch (Exception e){
                    finStatus=OrderStatus.FAIL.getCode();
                    //解冻
            ...
            criteria.andOrder_noEqualTo(rwWithdraw.getOrder_no());
            criteria.andEnabledEqualTo(Constants.ENABLED);
            ...
            logger.info("【平台提现】-->请求三方成功-->order_no:" + withdrawRequest.getOrder_no());
            if(!OrderStatus.REQUESTSUCCESS.getCode().equals(finStatus)){
                withdraw.setPay_status(finStatus);
                rwWithdrawMapper.updateByPrimaryKeySelective(withdraw);
            }
            if(OrderStatus.FAIL.getCode().equals(finStatus)){
                //限额回滚
                CacheUtil.getCache().increment(platWithdrawRedisKey,0-Long.parseLong(bd100.indexOf(".")>0?bd100.substring(0,bd100.indexOf(".")):bd100));
...
//        withdrawBussMap.put("order_no",trans_serial);
        ...
        withdrawBussMap.put("order_no",rwWithdraw.getOrder_no());
        withdrawBussMap.put("plat_no",rwWithdraw.getPlat_no());
        withdrawBussMap.put("notify",rwWithdraw.getNotify_url());
        withdrawBussMap.put("mall_no",accountSearchService.queryMallNoByPlatNo(rwWithdraw.getPlat_no()));

        TransTransreq transTransreq=transReqService.queryTransTransReqByOrderNoAndBatchOrderNo(rwWithdraw.getTrans_serial(),null);
        if(transTransreq==null){
            return null;
        }

        //平台提现解冻扣钱
        AccountSubjectInfo accountSubjectInfo =  accountQueryService.queryAccount(rwWithdraw.getPlat_no(),rwWithdraw.getPlatcust(),Tsubject.CASH.getCode(),Ssubject.PLAT.getCode());
        ...
        eaccAccountamtlist.setPlatcust(accountSubjectInfo.getPlatcust());
        eaccAccountamtlist.setSubject(Tsubject.CASH.getCode());
        eaccAccountamtlist.setSub_subject(rwWithdraw.getSub_subject());
        eaccAccountamtlist = accountTransferService.queryTransFlow(eaccAccountamtlist);
        ...
        if("Y".equals(status)){
            ...
            rwWithdraw.setUpdate_time(new Date());
            int isUpdate =rwWithdrawMapper.updateByPrimaryKey(rwWithdraw);
            ...
            //限额回滚
            String platWithdrawRedisKey=Constants.PLAT_WITHDRAW_DAY_LIMIT_KEY+rwWithdraw.getPlat_no()+DateUtils.getyyyyMMddDate();
            String bd100=String.valueOf(rwWithdraw.getOut_amt().multiply(new BigDecimal(100)));
            CacheUtil.getCache().increment(platWithdrawRedisKey,0-Long.parseLong(bd100.indexOf(".")>0?bd100.substring(0,bd100.indexOf(".")):bd100));
            withdrawBussMap.put("code","2");
            /*=============================处理失败业务=====================================*/
            rwWithdraw.setAcct_state(AcctState.Reach.getCode());
            rwWithdraw.setPay_status(PayStatus.FAIL.getCode());
            rwWithdrawMapper.updateByPrimaryKey(rwWithdraw);
        ...
        try{
            ...
            logger.error("【平台提现异步回调】异步通知发送失败");
        }
