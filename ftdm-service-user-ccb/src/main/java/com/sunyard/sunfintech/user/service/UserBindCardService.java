...
//            accountVerifyParams.put("verify_info",applyRequest.getIdentifying_code());
    ...
    public void getCode4Password(VcodeRequest vcodeRequest) throws Exception, BusinessException {
        ...
        if(StringUtils.isNotBlank(vcodeRequest.getMobile())){
            sendMsg.setMobile(vcodeRequest.getMobile());
        }
...
//                    msgModel.setPlatcust(eaccUserinfo.getMallcust());
            ...
            registerNotifyData.setPre_mobile(confirmRequest.getPre_mobile());
            registerNotifyData.setCard_no(confirmRequest.getCard_no());
...
//            transTransreq.setReturn_msg(BusinessMsg.getMsg(BusinessMsg.SUCCESS));
    ...
    public BaseResponse applyOrgRegisterOpening(ApplyOrgRegisterOpeningRequest applyRequests) throws BusinessException {
        ...
        try {
            ...
            /*  oldTransTransreq=transTransreq;*/
            //开户
            String platcust= applyOrgRegisterOpening(applyRequests,registerRequest,oldTransTransreq);
    ...
    private  String applyOrgRegisterOpening(ApplyOrgRegisterOpeningRequest applyRequests,CompanyRegisterRequest registerRequest,TransTransreq oldTransTransreq){
...
//            }
            ...
            if(CusType.PERSONAL.getCode().equals(bindCardRequest.getType())){
                ...
                if(IsBankCheck.NO.getCode().equals(platPaycode.getIs_bankcheck())){
                    ...
                    if(!(Boolean) noMsgBindCardResponse.get("success")){
                        logger.info("【无短验绑卡】=========绑卡失败："+noMsgBindCardResponse.get("error_info"));
                        throw new BusinessException(BusinessMsg.TIEDCARD_FAILED, BusinessMsg.getMsg(BusinessMsg.TIEDCARD_FAILED)+"："+noMsgBindCardResponse.get("error_info"));
                    }
                }else{
    ...
    public BaseResponse checkCard(CheckCardRequest checkCardRequest) throws BusinessException {
        ...
        try{
            ...
            statusList.add(CardStatus.UNACTIVE.getCode());
            List<EaccCardinfo> eaccCardinfoList=accountSearchService.queryEaccCardInfo(
                    checkCardRequest.getMall_no(),checkCardRequest.getPlatcust(),checkCardRequest.getCard_no(),statusList);
            if(eaccCardinfoList.size()>0){
                EaccCardinfo eaccCardinfo=eaccCardinfoList.get(0);
...
//            sendParams.put("partner_serial_no",transTransreq.getTrans_serial());
//            sendParams.put("partner_trans_date",bindCardRequest.getPartner_trans_date());
//            sendParams.put("partner_trans_time",bindCardRequest.getPartner_trans_time());
//            sendParams.put("client_name",bindCardRequest.getName());
//            sendParams.put("id_kind","2");
//            sendParams.put("id_no",bindCardRequest.getId_code());
//            sendParams.put("mobile_tel",bindCardRequest.getPre_mobile());
//            sendParams.put("card_no",bindCardRequest.getCard_no());
//            sendParams.put("func_code","3");
//            sendParams.put("sendercomp_id",platPaycode.getPayment_plat_no());
//            sendParams.put("targetcomp_id","91000");
//            sendParams.put("channelId",platPaycode.getChannel_id());
//            sendParams.put("pay_bankacct_type","0");
//            //=========雅酷必填字段============
//            sendParams.put("partner_userid",bindCardRequest.getRemark());
            //对公用户
            eaccCardinfo.setStatus(CardStatus.UNACTIVE.getCode());
            eaccCardinfo.setRemark(BindCardType.ORGBINGCARD.getCode());
            eaccCardinfo.setAcct_name(bindCardRequest.getAcct_name());
...
//        String auth_types=registerRequest.getAuthed_type().replace("，",",").trim();
        String role_types=registerRequest.getRole().replace("，",",").trim();
