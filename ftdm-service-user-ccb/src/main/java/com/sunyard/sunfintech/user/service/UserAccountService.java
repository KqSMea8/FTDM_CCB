...
//    }
    ...
    public ChangeAccountInfoResponse  changeAccountInfo(ChangeAccountInfoRequest changeAccountInfoRequest)throws BusinessException {
        ...
        try{
            ...
            if (CusType.COMPANY.getCode().equals(changeAccountInfoRequest.getCus_type())) {
                logger.info("【修改预留手机号】企业修改默认手机号");
                eaccUserInfo.setDefault_mobile(changeAccountInfoRequest.getMobile());
            }
    ...
    public boolean updateCardInfo(EaccCardinfo eaccCardInfo) throws BusinessException {
        try{
            ...
            if(null == eaccCardInfo.getUpdate_time()) eaccCardInfo.setUpdate_time(new Date());
    ...
    public boolean updateEaccUserInfo(EaccUserinfo eaccUserinfo, String mall_no, String mallcust) throws BusinessException {
        ...
        if(null == eaccUserinfo.getUpdate_time()) eaccUserinfo.setUpdate_time(new Date());
    ...
    @Override
    @Transactional/*分布式数据库不支持嵌套事物(propagation = Propagation.NESTED)*/
...
//            eaccCardInfoMapper.insert(eaccCardInfo);
    ...
    public PlatplatcustRegisterResponseData openEaccount(PlatplatcustRegisterRequest platplatcustRegister) throws BusinessException {
        ...
        try{
            ...
            Map<String,Object> platcustOpenResponse=adapterService.registerEaccount(params);
...
//            });
        ...
        try{
            if(transTransreq==null){
                ...
                if(setPasswordObj!=null){
                    ...
                    if(StringUtils.isBlank(setNewPasswordRequest.getName())){
                        setNewPasswordRequest.setName(setPwdRequest.getName());
                    }
                    if(StringUtils.isBlank(setNewPasswordRequest.getId_code())){
                        setNewPasswordRequest.setName(setPwdRequest.getId_code());
                    }
                    if(StringUtils.isBlank(setNewPasswordRequest.getPre_mobile())){
                        setNewPasswordRequest.setName(setPwdRequest.getMobile());
                    }
                }else{
            ...
            //验证账户状态
            EaccUserinfo eaccUserinfo=accountSearchService.queryEaccUserInfoByEaccAccountInfo(setNewPasswordRequest.getMall_no(),null,platcust);
            ...
            if(!Constants.YES.equals(eaccUserinfo.getPwd_flg())){
                ...
                //激活账户
                EaccCardinfo unActiveCardInfo=eaccCardinfoList.get(0);
                ...
                logger.info("【交易密码设置】setPwdRequest："+JSON.toJSONString(setPwdRequest));
                if(setPwdRequest!=null){
                    logger.info("【交易密码设置】设置授权："+JSON.toJSONString(setPwdRequest));
                    userBindCardService.addAuthInfo(setPwdRequest.getAuthed_type(),setPwdRequest.getAuthed_amount(),
                            setPwdRequest.getAuthed_limittime(),AuthStatus.ALREADY_AUTH.getCode(),
                            setPwdRequest.getMall_no(),setPwdRequest.getMer_no(),eaccUserinfo.getMallcust());
                    //设置角色，授权
                    logger.info("【交易密码设置】设置角色："+JSON.toJSONString(setPwdRequest));
                    String role_types=setPwdRequest.getRole().replace("，",",").trim();
                    String[] roleArray=role_types.split(",");
                    for(String role_type:roleArray){
                        if(RoleType.INVESTOR.getCode().equals(role_type)){
                            updateEaccuserinfo.setInvester(StringUtils.stringToByte(role_type));
                        }else if(RoleType.BORROWER.getCode().equals(role_type)){
                            updateEaccuserinfo.setFinancier(StringUtils.stringToByte(role_type));
                        }else if(RoleType.ADVANCER.getCode().equals(role_type)){
                            updateEaccuserinfo.setAdvancer(StringUtils.stringToByte(role_type));
                        }else if(RoleType.GUARANTOR.getCode().equals(role_type)){
                            updateEaccuserinfo.setGuarantor(StringUtils.stringToByte(role_type));
                        }
                    }
                }
                ...
                check4Element(setNewPasswordRequest.getMall_no(),platcust,
                        setNewPasswordRequest.getName(),setNewPasswordRequest.getId_code(),setNewPasswordRequest.getCard_no(),setNewPasswordRequest.getPre_mobile());
            ...
            sendParams.put("id_no",eaccUserinfo.getId_code());
    ...
    private EaccUserinfo check4Element(String mall_no,String platcust,String name,String id_code,String card_no,String pre_mobile){
        logger.info("【设置交易密码四要素验证】"+mall_no+"|"+platcust+"|"+name+"|"+id_code+"|"+card_no+"|"+pre_mobile);
        EaccUserinfo userinfo=accountSearchService.queryEaccUserInfoByEaccAccountInfo(mall_no,null,platcust);
        if(userinfo==null){
            throw new BusinessException(BusinessMsg.INVALID_ACCOUNT,BusinessMsg.getMsg(BusinessMsg.INVALID_ACCOUNT));
        }
        if(CusType.PERSONAL.getCode().equals(userinfo.getCus_type())){
            if(!name.equals(userinfo.getName())){
                throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|姓名错误");
            }
            if(!id_code.equals(userinfo.getId_code())){
                throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|证件号错误");
            }
        }else{
            if(!name.equals(userinfo.getOrg_name())){
                throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|企业名称错误");
            }
            if(!id_code.equals(userinfo.getBank_license()) && !id_code.equals(userinfo.getBusiness_license())){
                throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|企业证件号错误");
            }
        }
        List<EaccCardinfo> eaccCardinfoList=accountSearchService.queryEaccCardInfo(mall_no,platcust,card_no,AcctStatus.ACTIVE.getCode());
        EaccCardinfo eaccCardinfo=null;
        if(eaccCardinfoList.size()>0){
            eaccCardinfo=eaccCardinfoList.get(0);
        }
        if(eaccCardinfo==null){
            throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|卡号错误");
        }
        if(CusType.PERSONAL.getCode().equals(userinfo.getCus_type()) && !eaccCardinfo.getMobile().equals(pre_mobile)){
            throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|预留手机号错误");
        }else if(CusType.COMPANY.getCode().equals(userinfo.getCus_type()) && !userinfo.getMobile().equals(pre_mobile)){
            throw new BusinessException(BusinessMsg.FOUR_VERIFY_FAILED,BusinessMsg.getMsg(BusinessMsg.FOUR_VERIFY_FAILED)+"|手机号错误");
        }
