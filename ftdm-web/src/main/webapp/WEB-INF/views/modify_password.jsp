    ...
    <div id="kaihu_page" >
        ...
        <div class="list_page">
            <ul>
                ...
                <%--<li>--%>
                    <%--<p>证件类型</p>--%>
                    <%--<input type="text" name="id_type" value="身份证" readonly>--%>
                <%--</li>--%>
                <li>
                    <p>证件号码</p>
                    <input id="id_code" type="text" name="id_code" maxlength="18" value="" placeholder="${code_type}">
                </li>
        ...
        <div class="list_page">
            <ul>
                <li>
                    <p>银行卡号 </p>
                    <input id="card_no" type="text" name="card_no" value="" placeholder="银行卡号">
                </li>
                <li>
                    <p>${mobile_type}</p>
                    <input id="pre_mobile" type="text" name="pre_mobile" value="" maxlength="11" placeholder="预留手机号">
                </li>
                <li>
                    <p>短信验证码</p>
...
//                    contentType: "application/json;charset=UTF-8",
                ...
                submitFun: function (RootUrl) {
                    ...
                    if(name == undefined || id_code == undefined || card_no == undefined || pre_mobile == undefined){
                        var msg = "<div class=\"alert_box_layer\"><img src=\"${url}image/error.png\"><span>请将四要素信息填写完整</span></div>";
                        layer.msg(msg);
                        return;
                    }
                    params.name = name;
                    ...
                    params.trans_serial = "${data.transSerial}";
                    params.identifying_code = code;
                    params.token="${token}";
                    $(".submit_btn").removeAttr("onclick");
                    page.postRequest(RootUrl + "account/set_trade_password", params, function (res) {
                        $(".submit_btn").click(function () {
                            page.submitFun(RootUrl)
                        });
                        if (res.recode == 10000) {
                            location.href = RootUrl + "account/update_password_success" + page.encodeParams({trans_serial:"${data.transSerial}",token:"${token}"});
                        } else {
            ...
            codeFun: function (RootUrl) {
                var pre_mobile = $('#pre_mobile').val();
                if(pre_mobile == undefined || pre_mobile.length < 11){
                    var msg = "<div class=\"alert_box_layer\"><img src=\"${url}image/error.png\"><span>请输入正确的手机号码</span></div>";
                    layer.msg(msg);
                    return;
                }
                $('#send_auth_code').attr("disabled", true).siblings('span').text("正在发送");
                    ...
                    params.trans_serial = "${data.transSerial}";
                    params.origin_order_no = "${data.order_no}";
                    params.mobile = pre_mobile;
                    params.token="${token}";

                    page.postRequest(RootUrl + "account/get_code_4_set_password", params, function (res) {
                        if (res.recode == "10000") {
                            var _no = 60;
