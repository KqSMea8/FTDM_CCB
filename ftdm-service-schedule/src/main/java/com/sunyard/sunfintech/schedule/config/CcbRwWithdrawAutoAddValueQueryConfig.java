 ...
 */
@Component
public class CcbRwWithdrawAutoAddValueQueryConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "ccbRwWithdrawAutoAddValueQuery";
        try {
...
//        baseTaskType.setSleepTimeInterval(60 * 1000);//每处理完一批数据后休眠时间
        ...
        baseTaskType.setDealBeanName("ccbRwWithdrawAutoAddValueQueryBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("自动加值状态查询");
