 ...
 */
@Component
public class PayingWithdrawBean implements IScheduleTaskDealSingle<RwWithdraw> {
    ...
    public boolean execute(RwWithdraw task, String s) throws Exception {
        try {
            logger.info("【处理中提现】-->开始执行-->订单号：" + task.getOrder_no());
            rwWithdrawOptionService.doThirdpartyQueryRwWithdraw(task);
        }catch (Exception e){
            logger.info("【处理中提现】-->异常-->订单号：" + task.getOrder_no(),e);
        }
        return true;
    }
    ...
    public List<RwWithdraw> selectTasks(String s, String s1, int i, List<TaskItemDefine> list, int catchNum, int i2) throws Exception {
        ...
        try {
            withdrawList = rwWithdrawOptionService.queryRwWithdrawByPaying(catchNum);
        }catch (Exception e){
            logger.info("【处理中提现】查询处理中的提现时异常",e);
        }
        return withdrawList;
    }
