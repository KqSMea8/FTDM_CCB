 ...
 */
@Component
public class CcbRwWithdrawAutoAddValueConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "ccbRwWithdrawAutoAddValue";
        try {
        ...
        baseTaskType.setPermitRunStartTime("0 0/10 * * * ?");//调度任务开始时间
...
//        baseTaskType.setSleepTimeInterval(60 * 1000);//每处理完一批数据后休眠时间
        ...
        baseTaskType.setDealBeanName("ccbRwWithdrawAutoAddValueBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("自动加值");
