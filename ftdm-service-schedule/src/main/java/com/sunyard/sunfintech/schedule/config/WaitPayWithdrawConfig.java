 ...
 */
@Component
public class WaitPayWithdrawConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "waitPayWithdraw";
        try {
...
//        baseTaskType.setSleepTimeInterval(60 * 1000);//每处理完一批数据后休眠时间
        ...
        baseTaskType.setDealBeanName("waitPayWithdrawBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("查询待处理提现");
