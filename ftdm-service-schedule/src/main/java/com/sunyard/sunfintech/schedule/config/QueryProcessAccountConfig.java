 ...
 */
@Component
public class QueryProcessAccountConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "queryProcessAccount";
        try {
...
//        baseTaskType.setSleepTimeInterval(60 * 1000);//每处理完一批数据后休眠时间
        ...
        baseTaskType.setDealBeanName("queryProcessAccountBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("查询account中订单");
