 ...
 */
@Component
public class CcbWaitSendWithdrawConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "ccbWaitSendWithdraw";
        try {
        ...
        //提现代发早上9点半开启，晋商7*24小时
        baseTaskType.setPermitRunStartTime("0 0/3 * * * ?");//调度任务开始时间
        //提现代发晚上17点结束，晋商7*24小时
//        baseTaskType.setPermitRunEndTime("0 0 17 * * ?");//调度任务结束时间
//        baseTaskType.setSleepTimeNoData(1000 * 60 * 2);//采取不到数据休眠时间
//        baseTaskType.setSleepTimeInterval(60 * 1000);//每处理完一批数据后休眠时间
        baseTaskType.setFetchDataNumber(100);//每次采集任务数量
        ...
        baseTaskType.setDealBeanName("ccbWaitSendWithdrawBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("提现代发");
