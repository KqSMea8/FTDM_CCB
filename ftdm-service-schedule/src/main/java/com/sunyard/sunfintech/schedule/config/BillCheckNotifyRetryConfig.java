...
/**
 * 每天对账完成后发送通知给中间平台，如果有发送失败的通知该任务才会起作用
 */
@Component
public class BillCheckNotifyRetryConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "billCheckNotifyRetry";
        try {
        ...
        baseTaskType.setPermitRunStartTime("0 0/5 * * * ?");//调度任务开始时间
//        baseTaskType.setPermitRunEndTime("0 0 21 * * ?");//调度任务结束时间
//        baseTaskType.setSleepTimeNoData(1000 * 60 * 2);//采取不到数据休眠时间
        baseTaskType.setSleepTimeInterval(60 * 5);//每处理完一批数据后休眠时间
        baseTaskType.setFetchDataNumber(500);//每次采集任务数量
        ...
        baseTaskType.setDealBeanName("billCheckNotifyRetryBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("清算成功通知行内");
