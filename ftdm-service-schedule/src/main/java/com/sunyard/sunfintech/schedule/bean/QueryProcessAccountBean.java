 ...
 */
@Component
public class QueryProcessAccountBean implements IScheduleTaskDealSingle<TransTransreq> {
    ...
    public boolean execute(TransTransreq task, String ownSign) throws Exception {
        try {
            logger.info(String.format("【开始执行长时间处理中的转账订单】流水号：%s", task.getTrans_serial()));
            accountServiceStatusQueryService.doProcessingOrder(task);
        } catch (Exception e) {
            logger.error("定时任务处理流水号为：" + task.getTrans_serial() + "的转账订单异常：", e);
            return false;
        }
        return true;
    }
    ...
    public List<TransTransreq> selectTasks(String taskParameter, String ownSign, int taskItemNum, List<TaskItemDefine> taskItemList, int eachFetchDataNum, int pageNum) throws Exception {
        logger.info("=====开始查询处理中的Account订单=====");
        List<TransTransreq> transTransreqs = null;
        try {
            transTransreqs = accountServiceStatusQueryService.queryMyProcessingOrder(eachFetchDataNum);
        } catch (Exception e) {
            logger.info("【查询处理中的标的Account失败】");
        }
