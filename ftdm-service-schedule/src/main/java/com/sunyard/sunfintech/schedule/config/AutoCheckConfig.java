 ...
 */
@Component
public class AutoCheckConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        String baseTaskTypeName = "autoCheck";
        try {
        ...
        //每周二到周五的8点到10点每隔20分钟执行
        baseTaskType.setPermitRunStartTime("0 0/20 8-10 ? * TUE-FRI");//调度任务开始时间
...
//        baseTaskType.setSleepTimeInterval(1000);//每处理完一批数据后休眠时间
        ...
        baseTaskType.setDealBeanName("autoCheckBean");
//        baseTaskType.setProcessorType("SLEEP");//处理器类型(SLEEP--NOSLEEP) 默认SLEEP
        ...
        strategy.setTaskParameter("自动对账");
